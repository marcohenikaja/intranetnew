...
const Cadrenomanagerpdf = () => {
    ...
    const validerEvaluation = async () => {
        ...
        if (emailn2 === "" || emaildr === "") {
            ...
            notification.info({
                message: `info`,
                description: "E-mail Directeur Direction vide",
                placement,
            });
            return 0;
        }
        try {
            const enrg = await axios.post(`${url}enregistrementevalcadrenonmanager/${id}`, {
                ...
                ct1, ct2, ct3, mt1, mt2, mt3, ml1, ml2, ml3, cpr1, cpr2, cpr3, cg1, cg2, cg3, comcollab, objectifs1, resultat1, somme, todayis, alp1, alp2, emailn1, emailn2, emaildr, emailsg, emaildg, emaildrh, loggedInUser, ids, loggedInUser
            });
    ...
    const getAlldataevaluationnonmanager = async () => {
        try {
            const response = await axios.get(`${url}getAlldataevaluationcadrenonmanager/${id}/${type}`);
    ...
    const etape6 = (placement) => {
        ...
        if (!allChecked) {
            ...
            const value3 = parseFloat(selectedVal3) || 0;
            const value14 = parseFloat(selectedVal14) || 0;
            const value15 = parseFloat(selectedVal15)
            const value0 = parseFloat(selectedVal6) || 0;

            const v1 = parseFloat(selectedVal1) || 0;
            const v2 = parseFloat(selectedVal2) || 0;
            const v3 = parseFloat(selectedVal4) || 0;
            ...
            const R1 = (value1 + value2 + intermediateResult + value3 + value14 + value15) / 6;

            const value6 = parseFloat(selectedVal9) || 0;
            const v7 = parseFloat(selectedVal7) || 0;

            const v9 = parseFloat(selectedVal9) || 0;
            ...
            const v4 = parseFloat(selectedVal5) || 0;
            const v5 = parseFloat(selectedVal8) || 0;
            const v6 = parseFloat(selectedVal10) || 0;
    ...
    useEffect(() => {
        ...
        setSommes(sum);
    }, [selectedVal1, selectedVal2, selectedVal3, selectedVal4, selectedVal5, selectedVal6, selectedVal7, selectedVal8, selectedVal9, selectedVal10, selectedVal14, selectedVal15]);
    ...
    const enregistrement = async () => {
        try {
            const enrg = await axios.post(`${url}enregistrementevalcadrenonmanager/${ids}`, {
                ...
                t1, t2, t3, t4, compac1, compac2, compac3, compac4, apav1, apav2, apav3, apav4, apap1, apap2, apap3, apap4, comm1, comm2, comm3, comm4,
                ccd1, ccd2, ccd3, ccd4, catcomp1, catcomp2, catcomp3, catcomp4, motif1, motif2, motif3, motif4, pa1, pa2, pa3, pa4, dp1, dp2, dp3, dp4,
                ct1, ct2, ct3, mt1, mt2, mt3, ml1, ml2, ml3, cpr1, cpr2, cpr3, cg1, cg2, cg3, comcollab, objectifs1, resultat1, somme, todayis, alp1, alp2, emailn1, emailn2, emaildr, emailsg, emaildg, emaildrh, ids
            });
                                        ...
                                        >
                                            <div style={{ marginBottom: '10px', marginTop: '50px' }}>
                                                <AutoComplete
                                                    style={{ width: '100%' }}
                                                    options={options2}
                                                    onSearch={handleSearchn2}
                                                    onSelect={handleSelectn2}
                                                    onChange={handleChangen2}
                                                    value={emailn2}
                                                    disabled={loggedInUser !== emailn1}
                                                >
                                                    <Input
...
export default Cadrenomanagerpdf; // Assurez-vous que cette ligne est présente pour exporter le composant Cadrenomanagerpdf
